...
describe('applyPermanentBonuses', () => {
  it('aggregates permanent bonuses from record purchases', () => {
    ...
    expect(bonuses.lampotilaRateMult).toBeCloseTo(1 + 0.2 * 3, 6);
  ...
  it('handles array-style purchase tracking', () => {
    ...
    expect(bonuses.techMultiplierBonusAdd).toBeCloseTo(0.5 * 3, 6);
    expect(bonuses.globalCpsAddPerTuhkaSpent).toBeCloseTo(0.1 * 2, 10);
    ...
    expect(bonuses.globalCpsAddFromTuhkaSpent).toBeCloseTo(expectedSpent * 0.1 * 2, 6);
    expect(bonuses.globalMultPerBuilding).toBeCloseTo(0, 6);
  });
